 ...
 */
async function triggerRunScriptDialog(commandEngine, autoSync = false) {
    const html = `
    ...
    openModal('<i class="fa-solid fa-code"></i> Run .sim Script', html, () => {
        ...
        async function runScript(code) {
            ...
            const res = await commandEngine.executeScript(code);
            console.log(res);
            if (res.success) {
                commandEngine.engine.evaluateAll();
                selectionManager.clear();
...
function setupUIEvents(commandEngine) {
    ...
    document.getElementById("btn-run-script").addEventListener("click", async () => {
        await triggerRunScriptDialog(commandEngine, false);
    });

    document.getElementById("btn-sync-canvas").addEventListener("click", async () => {
        await triggerRunScriptDialog(commandEngine, true);
    });
